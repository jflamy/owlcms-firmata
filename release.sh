...
#
# Usage:
#   VERSION=2.6.0-rc01 ./release.sh
#   ./release.sh 2.6.0-rc01
#
# Defaults:
#   - Commits + pushes release note sources (RELEASE.md, release.sh) before triggering
#     (so CI builds what you see)
#   - Runs the workflow on the current git branch
#   - After a successful run, does a safe git pull --ff-only

if [[ -z "${VERSION}" ]]; then
  echo "ERROR: VERSION must be set (e.g. VERSION=2.6.0-rc01)" >&2
  exit 1
fi
...
if git rev-parse "${VERSION}" >/dev/null 2>&1; then
  echo "ERROR: Tag '${VERSION}' already exists in local repository." >&2
  echo "       Use a new version number." >&2
  exit 3
fi

if git ls-remote --tags origin | grep -q "refs/tags/${VERSION}$"; then
  echo "ERROR: Tag '${VERSION}' already exists in remote repository." >&2
  echo "       Use a new version number." >&2
  exit 3
fi
...
ARGS=(--repo "${REPO}" -f "version=${VERSION}")
ARGS+=( --ref "${GIT_REF}" )
